...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
          if maxActionValue < self.getQValue(state, action):
            maxActionValue = self.getQValue(state, action)
            maxAction = action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for f in self.featExtractor.getFeatures(state, action):
          total += self.weights[f] * self.featExtractor.getFeatures(state, action)[f]
        return total
    ...
    def update(self, state, action, nextState, reward):
        ...
        difference = (reward + self.discount * maxAction) - self.getQValue(state, action)
        for f in features:
