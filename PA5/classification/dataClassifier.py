...
def enhancedFeatureExtractorDigit(datum):
    ...
    HALF_HEIGHT = DIGIT_DATUM_HEIGHT / 2
    HALF_WIDTH = DIGIT_DATUM_WIDTH / 2
        ...
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y) > 0:
                ...
                if y < HALF_HEIGHT:
                    ...
                    upperHalf += 1
    percentUpper = upperHalf / HALF_HEIGHT * HALF_WIDTH
    percentLower = lowerHalf / HALF_HEIGHT * HALF_WIDTH
    percentLeft = leftHalf / HALF_WIDTH * HALF_HEIGHT
    percentRight = rightHalf / HALF_HEIGHT * HALF_WIDTH

    for i in range(10): 
        if percentUpper > 0.25:
            features["up" + str(i)] = 1;
        else:
            features["up" + str(i)] = 0;

        if percentLower > 0.25:
            features["lower" + str(i)] = 1;
        else:
            features["lower" + str(i)] = 0;

        if percentLeft > 0.25:
            features["left" + str(i)] = 1;
        else:
            features["left" + str(i)] = 0;

        if percentRight > 0.25:
            features["right" + str(i)] = 1;
        else:
            features["right" + str(i)] = 0;
