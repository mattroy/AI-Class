...
def genericSearch(problem, queue, heuristic=nullHeuristic):
    ...
    while not queue.isEmpty():
        nextState, path, cost, estimatedCost = queue.pop()
        
        if not visitedStates[nextState]:
            visitedStates[nextState] += 1
            if problem.isGoalState(nextState):
                return path

            for newState, direction, nextCost in problem.getSuccessors(nextState):
                newPath = list(path)
                newPath.append(direction)
                queue.push((newState, newPath, cost + nextCost, cost + nextCost + heuristic(newState, problem)))
...
def heuristicCost(state):
    """Retrieve the heuristic from the state"""
    position, path, cost, heuristic = state
    return heuristic
